...
- Section hierarchy (`scripts/hierarchy_audit.py`, 2026-09-16): 0.6% of checkable accounts misparented against USASpending truth, no agency nested under another, 1 parenthetical subtitle parsed as a section, and 1,648 duplicate paths (from 131, 1,895, and 4,020 before the hierarchy fixes); table-row leakage fell from 372 to 130 paragraphs. Details in `docs/methodology.md`.
- Cross-year alignment (`scripts/linkage_eval.py score`): about 100% precision over all labeled candidates, and 87.5% (21 of 24) over the adjudicated borderline candidates that still match after the hierarchy fixes (86.5%, 32 of 37, before them); rename promotion is 4 of 6. A segmentation-independent check agrees with 68 of the 78 labeled passages still locatable. Blocking misses are not measured. Details in `docs/methodology.md`.
